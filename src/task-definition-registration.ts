...
function readTaskDefinitionTemplate(input: TaskRegistrationInput): RegisterTaskDefinitionRequest | undefined  {
...
function overrideContainerImages(definition: RegisterTaskDefinitionRequest, containerImages: object): void {
...
function overrideEnvironmentVars(definition: RegisterTaskDefinitionRequest, environmentVars: object): void {
  const { containerDefinitions } = definition
...
function processTaskDefinitionInput(input: TaskRegistrationInput): RegisterTaskDefinitionRequest | undefined {
...
export async function registerTaskDefinition(input: TaskRegistrationInput): Promise<TaskDefinition> {
  const ecs = getClient()
  const taskDefinitionToRegister = processTaskDefinitionInput(input)
  if (!taskDefinitionToRegister) throw new Error("No Task Definition for given template")
